...
void Account::updateDependentAccount(LinphoneRegistrationState state, const std::string &message) {
	...
	for (;it;it = it->next) {
		...
		if (tmp != this->toC() && linphone_account_get_dependency(tmp) == this->toC()) {
			...
			auto copyParams = mParams->clone()->getSharedFromThis();
			if (state == LinphoneRegistrationOk) {
				// Force dependent account to re-register
				mParams->mRegisterEnabled = false;
...
void Account::resolveDependencies () {
	...
	for(elem = mCore->sip_conf.accounts; elem != NULL; elem = elem->next) {
		...
		if (dependency != NULL && !mParams->mDependsOn.empty()) {
			LinphoneAccount *master_account = linphone_core_get_account_by_idkey(mCore, mParams->mDependsOn.c_str());
		...
		if (!mParams->mDependsOn.empty() && dependency == NULL) {
			LinphoneAccount *dependency_acc = linphone_core_get_account_by_idkey(mCore, mParams->mDependsOn.c_str());

			if (dependency_acc == NULL) {
				ms_warning("LinphoneAccount marked as dependent but no account found for idkey [%s]", mParams->mDependsOn.c_str());
				return;
			} else {
				ms_message("LinphoneAccount [%p] now depends on master LinphoneAccount [%p]", account, dependency_acc);
				linphone_account_set_dependency(account, linphone_account_ref(dependency_acc));
			}
		}
	}
}
